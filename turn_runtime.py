            ...
            if status_match:
                ...
                if perceived_text.strip() and len(display_response.strip()) > 2:
                    with st.spinner("系统感知到剧情突变，动态图谱落盘中..."):
                        from core_engine import sync_dynamic_status
                        st.session_state.major_graph, raw_json = sync_dynamic_status(
                            perceived_text,
                ...
                if display_response and len(display_response.strip()) > 2:
                    with st.spinner("战后伤情与状态落盘中..."):
                        from core_engine import sync_dynamic_status
                        st.session_state.major_graph, raw_json_combat = sync_dynamic_status(
                            display_response,
