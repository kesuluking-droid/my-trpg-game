...
# ============================================================================
# 意图解析结果缓存（减少重复的 LLM 调用）
# ============================================================================
# 缓存结构: {cache_key: {"result": {...}, "scene_hash": str}}
_intent_cache: dict = {}
_INTENT_CACHE_MAX_SIZE = 200  # 最大缓存条目数
_INTENT_CACHE_TTL = 3600  # 缓存有效期（秒）


def _generate_intent_cache_key(user_input: str, active_scene: list) -> str:
    """生成意图解析缓存键"""
    # 缓存键 = 玩家输入哈希 + 最近2条对话哈希
    input_hash = hashlib.md5(user_input.encode('utf-8')).hexdigest()[:12]
    
    # 取最近2条对话作为上下文标识
    recent = active_scene[-2:] if active_scene else []
    scene_str = json.dumps(recent, ensure_ascii=False, sort_keys=True)
    scene_hash = hashlib.md5(scene_str.encode('utf-8')).hexdigest()[:8]
    
    return f"{input_hash}_{scene_hash}"


def _get_cached_intent(user_input: str, active_scene: list) -> dict | None:
    """从缓存获取意图解析结果"""
    cache_key = _generate_intent_cache_key(user_input, active_scene)
...
def _set_cached_intent(user_input: str, active_scene: list, result: dict):
    """设置意图解析结果缓存"""
    cache_key = _generate_intent_cache_key(user_input, active_scene)
    ...
    if len(_intent_cache) >= _INTENT_CACHE_MAX_SIZE:
        ...
        del _intent_cache[first_key]
    
    _intent_cache[cache_key] = {
        "result": deepcopy(result),  # 深拷贝避免外部修改
    }
...
def parse_and_adjudicate_intent(
    ...
    # ============================================================================
    # 【缓存优化】检查是否已有缓存的意图解析结果
    # ============================================================================
    cached_parsed = _get_cached_intent(user_input, active_scene)
    
    if cached_parsed:
        # 【缓存命中】使用缓存的解析结果，跳过 LLM 调用
        parsed = cached_parsed
        raw_llm_json = None  # 缓存命中时无原始 JSON
    else:
        # 【缓存未命中】呼叫大模型解析
        parsed, raw_llm_json = call_llm_parse_intent(user_input, active_scene, graph)
        # 【缓存存储】保存解析结果
        _set_cached_intent(user_input, active_scene, parsed)
    ...
    if healed_items or lost_items:
        graph.major_graph = ensure_item_instances(graph.major_graph)
